...
def _bwd_kernel(
    ...
    V += off_z * stride_kz + off_h * stride_kh
    DO += off_z * stride_qz + off_h * stride_qh
    DQ += off_z * stride_qz + off_h * stride_qh
    DK += off_z * stride_kz + off_h * stride_kh
    DV += off_z * stride_kz + off_h * stride_kh
    ...
    # start_m = num_block_q * BLOCK_M
    # for tmp_m in range(lo, num_block_q * BLOCK_M, BLOCK_M):
    #     start_m -= BLOCK_M

    num_block_q = (N_CTX + BLOCK_M - 1) // BLOCK_M
    ...
    dv_ptrs = DV + (offs_n[:, None] * stride_kn + offs_k[None, :] * stride_kk)
    tl.store(dk_ptrs, dk)
    tl.store(dv_ptrs, dv)
...
BATCH, N_HEADS, HEAD_DIM = 1, 1, 128
